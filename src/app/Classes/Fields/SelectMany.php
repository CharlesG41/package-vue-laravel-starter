...
class SelectMany extends BaseField implements FieldInterface, \JsonSerializable
{
    const TYPE = 'select_many';
    ...
    static public function instantiateFromEloquentField(EloquentField $eloquentField, ?int $entryId): self
    {
        ...
        $field = new self(
            ...
            json_decode($fieldAttributesAsKeyValue['options'], true),
            $conditions,
            $fieldPermissions,
    ...
    public function createFieldValueInDatabase(FieldValueRepository $fieldValueRepository, array $localesByCode, ?int $fieldValueId, int $entryId)
    {
        ...
        if ($this->translatable) {
            foreach ($this->value as $localeCode => $values) {
                $locale = $localesByCode[$localeCode];
                $eloquentFieldValue = $fieldValueRepository->createFieldValue($this->id, $fieldValueId, $entryId, $locale->id, $this->key, null);
                foreach($values as $value) {
                    $eloquentFieldValue = $fieldValueRepository->createFieldValue($this->id, $eloquentFieldValue->id, $entryId, $locale->id, $this->key, $value);
                }
            }
        } else {
            $eloquentFieldValue = $fieldValueRepository->createFieldValue($this->id, $fieldValueId, $entryId, null, $this->key, null);
            foreach($this->value as $value) {
                $eloquentFieldValue = $fieldValueRepository->createFieldValue($this->id, $eloquentFieldValue->id, $entryId, null, $this->key, $value);
            }
        }
    }
