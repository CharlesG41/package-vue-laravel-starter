...
class Email extends BaseField implements FieldInterface, \JsonSerializable
{
    const TYPE = 'email';
    ...
    public function isValid($value): bool
    {
        if ($value && !filter_var($value, FILTER_VALIDATE_EMAIL)) {
            $this->error = __('cyvian.errors.email');
            return false;
        }

        if ($this->minimum && strlen($value) < $this->minimum) {
            $this->error = __('cyvian.errors.minimum', ['min' => $this->minimum]);
            return false;
        }

        if ($this->maximum && strlen($value) > $this->maximum) {
