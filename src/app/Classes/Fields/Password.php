...
class Password extends BaseField implements FieldInterface, \JsonSerializable
{
    const TYPE = 'password';
    ...
    static public function instantiateFromArray(array $data): self
    {
        ...
        $field = new self(
            ...
            $data['width'],
            $data['regex'],
            new Localisation($data['regex_message']),
            $data['minimum'],
            $data['maximum'],
            $conditions,
            $fieldPermissions,
    ...
    static public function instantiateFromEloquentField(EloquentField $eloquentField, ?int $entryId): self
    {
        ...
        $field = new self(
            ...
            $eloquentField->width,
            $fieldAttributesAsKeyValue['regex'],
            new Localisation(json_decode($fieldAttributesAsKeyValue['regex_message'], true)),
    ...
    public function createFieldValueInDatabase(FieldValueRepository $fieldValueRepository, array $localesByCode, ?int $fieldValueId, int $entryId)
    {
        ...
        if ($this->translatable) {
            foreach ($this->value as $localeCode => $value) {
                ...
                $fieldValueRepository->createFieldValue($this->id, $fieldValueId, $entryId, $locale->id, Hash::make($value));
            }
        } else {
            $fieldValueRepository->createFieldValue($this->id, $fieldValueId, $entryId, null, Hash::make($this->value));
        }
    }
    ...
    public function isValid($value): bool
    {
        ...
        if ($this->regex !== null && !preg_match('/' . $this->regex . '/', $value)) {
            $this->error = $this->regexMessage->getCurrent();
            return false;
        }
