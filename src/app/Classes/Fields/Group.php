...
use Cyvian\Src\app\Handlers\Utils\ValidationStore;
...
class Group extends BaseField implements FieldInterface, \JsonSerializable
{
    const TYPE = 'group';
    ...
    )
    {
        foreach ($fields as $field) {
            if (!$field instanceof FieldInterface) {
                throw new \InvalidArgumentException('Field must be implement FieldInterface');
            }
        }
        $this->fields = $fields;
    ...
    public function createFieldValueInDatabase(FieldValueRepository $fieldValueRepository, array $localesByCode, ?int $fieldValueId, int $entryId)
    {
        ...
        $mergeValueIntoFields = new MergeValueIntoField();

        $parentFieldValue = $fieldValueRepository->createFieldValue($this->id, $fieldValueId, $entryId, null, $this->key);
        $fields = $mergeValueIntoFields->handle($this->value, $this->fields);
    ...
    {
        foreach ($this->fields as $field) {
            $field->deleteFieldValueInDatabase($fieldValueRepository, $entryId);
        }
    }
    ...
    public function isValid($value): bool
    {
        ...
        $fields = $mergeValueIntoField->handle($value, $this->fields);
